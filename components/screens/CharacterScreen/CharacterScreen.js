...
export default function CharacterScreen() {
  const {
    ...
    effects, setEffects,
    equippedWeapons, setEquippedWeapons, // ← ДОБАВЛЕНО: отсутствующие переменные для экипировки
    equippedArmor, setEquippedArmor,     // ← ДОБАВЛЕНО: отсутствующие переменные для брони
    caps, setCaps,
  ...
  console.log('[CharacterScreen] Debug: setEquippedWeapons function:', typeof setEquippedWeapons);
  console.log('[CharacterScreen] Debug: setEquippedArmor function:', typeof setEquippedArmor);
  console.log('[CharacterScreen] Debug: equippedWeapons state:', equippedWeapons);
  console.log('[CharacterScreen] Debug: equippedArmor state:', equippedArmor);
  ...
  const handleSavePerkAttributes = () => {
    if (remainingPerkPoints > 0) {
      showAlert("Ошибка", "Вы должны распределить все доступные очки атрибутов.");
      return;
    }
  ...
  // ← ИСПРАВЛЕННАЯ ФУНКЦИЯ: Добавляем предметы из комплекта только в инвентарь
  const handleSelectKit = (kit) => {
    console.log('[CharacterScreen] Adding kit items to inventory:', kit.items.length, 'items');

    // ← ДОПОЛНИТЕЛЬНЫЕ ЛОГИ ДЛЯ ДИАГНОСТИКИ
    console.log('[CharacterScreen] Kit contents:', kit.items.map(item => ({
      name: item.Name || item.name,
      itemType: item.itemType,
      quantity: item.quantity || 1
    })));

    // Добавляем каждый предмет ОТДЕЛЬНО в инвентарь (без экипировки)
    kit.items.forEach(item => {
      console.log('[CharacterScreen] Adding item to inventory:', {
        name: item.Name || item.name,
        itemType: item.itemType,
        quantity: item.quantity || 1
      });

      // ← ИСПРАВЛЕНИЕ: Проверяем, что предмет не экипируется автоматически
      const existingCount = equipment?.items?.reduce((total, existingItem) => {
        const matches = existingItem.instanceId === item.instanceId ||
                       existingItem.uniqueId === item.uniqueId ||
                       (existingItem.Name || existingItem.name) === (item.Name || item.name);
        return matches ? total + (existingItem.quantity || 1) : total;
      }, 0) || 0;

      adjustItemQuantity(item, item.quantity || 1);

      console.log('[CharacterScreen] Item added successfully:', {
        name: item.Name || item.name,
    ...
    // Добавляем крышки
    if (kit.caps > 0) {
      console.log('[CharacterScreen] Adding caps:', kit.caps);
      setCaps(prev => prev + (kit.caps || 0));
    }

    // ← ИСПРАВЛЕНИЕ: НЕ экипируем предметы автоматически
    console.log('[CharacterScreen] Kit items added to inventory only - no auto-equip');

    // Закрываем модаль
    setIsEquipmentKitModalVisible(false);

    // Показываем уведомление
    showAlert("Готово", `Добавлено ${kit.items.length} предметов в инвентарь`);
  };
  ...
  const handleToggleSkill = (skillName) => {
    ...
    if (isForcedSkill && isCurrentlySelected) {
      showAlert("Ошибка", "Нельзя снять выбор с обязательного навыка");
      return;
    }
    ...
    if (!isCurrentlySelected) {
      ...
      if (currentSkill.value + 2 > skillMax) {
        showAlert("Ошибка", `Отметка этого навыка превысит максимальный ранг (${skillMax}). Сначала понизьте его значение.`);
        return;
      }
  ...
  const handleChangeSkillValue = (index, delta) => {
    ...
    if (delta > 0 && skillPointsLeft <= 0) {
      showAlert("Ошибка", "У вас не осталось очков навыков для распределения.");
      return;
    }
  ...
  const handleTraitPress = () => {
    if (!origin) {
      showAlert("Ошибка", "Сначала выберите происхождение");
      return;
    }
  ...
  const handleSaveAttributes = () => {
    ...
    // ← ИСПРАВЛЕНИЕ: Убираем проверку на equipment - теперь предметы в инвентаре
    if (remainingAttributePoints !== 0) {
      showAlert("Ошибка", "Потратьте все очки атрибутов перед сохранением.");
      return;
    }

    // Устанавливаем текущее здоровье равным максимальному после сохранения атрибутов
  ...
  const handleSaveSkills = () => {
    ...
    if (skillPointsLeft > 0) {
      showAlert("Ошибка", "Необходимо распределить все очки навыков.");
      return;
    }
    ...
    if (selectedSkills.length !== BASE_TAGGED_SKILLS) {
      showAlert("Ошибка", `Необходимо выбрать ровно ${BASE_TAGGED_SKILLS} основных навыка. Выбрано: ${selectedSkills.length}`);
      return;
    }

    // Проверяем экстра навыки от черт
    if (extraSkillsFromTrait > 0 && extraTaggedSkills.length !== extraSkillsFromTrait) {
      showAlert("Ошибка", `Необходимо выбрать ${extraSkillsFromTrait} дополнительных навыка от черты. Выбрано: ${extraTaggedSkills.length}`);
      return;
    }
  ...
  const handleEquipmentPress = () => {
    if (origin && origin.equipmentKits) {
      if (equipment) {
        // ← ИСПРАВЛЕНИЕ: Проверяем, действительно ли есть выбранное снаряжение
        const hasEquippedItems = equippedWeapons.some(w => w !== null) ||
                                Object.values(equippedArmor).some(slotData =>
                                  slotData.armor !== null || slotData.clothing !== null
                                );

        console.log('[CharacterScreen] Debug: Equipment check:', {
          equipment: !!equipment,
          hasEquippedWeapons: equippedWeapons.some(w => w !== null),
          hasEquippedArmor: Object.values(equippedArmor).some(slotData =>
            slotData.armor !== null || slotData.clothing !== null
          ),
          shouldShowWarning: hasEquippedItems
        });

        if (hasEquippedItems) {
          // Если есть надетые предметы, показываем предупреждение
          showConfirm(
            "Внимание!",
            "Инвентарь и всё снаряжение будет сброшено. Продолжить?",
            () => {
              // ← ЛОГИ ДЛЯ ВАЛИДАЦИИ ИСПРАВЛЕНИЯ
              console.log('[CharacterScreen] Debug: Resetting equipment, setEquippedWeapons type:', typeof setEquippedWeapons);
              console.log('[CharacterScreen] Debug: Resetting equipment, setEquippedArmor type:', typeof setEquippedArmor);
              console.log('[CharacterScreen] Debug: Current equippedWeapons before reset:', equippedWeapons);
              console.log('[CharacterScreen] Debug: Current equippedArmor before reset:', equippedArmor);
              ...
              // ← ДОПОЛНИТЕЛЬНЫЕ ЛОГИ ПОСЛЕ СБРОСА
              console.log('[CharacterScreen] Debug: Equipment reset completed');
              setCaps(0);
              setEquipment(null);
          ...
          // Если нет надетых предметов, просто открываем модаль выбора комплекта
          console.log('[CharacterScreen] Debug: No equipped items found, opening kit modal directly');
          setIsEquipmentKitModalVisible(true);
        }
      } else {
        console.log('[CharacterScreen] Debug: No equipment selected, opening kit modal');
        setIsEquipmentKitModalVisible(true);
      }
    } else {
      showAlert("Информация", "Для данного происхождения нет комплектов снаряжения.");
    }
  };
              ...
              <View style={styles.section}>
                ...
                <DerivedRow 
                  ...
                  value={(() => {
                    ...
                    const totalSelected = selectedSkills.length + extraTaggedSkills.length; // Исправлено: добавлена длина extraTaggedSkills
        ...
        <OriginModal
          ...
          onConfirm={() => {
            if (selectedOrigin) {
              ...
              showAlert("Ошибка", "Выберите происхождение");
            }
          }}
        />
